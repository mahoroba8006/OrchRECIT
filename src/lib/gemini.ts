...
const getReceiptPrompt = (mode: 'total' | 'details', customPrompt: string = '') => {
    ...
    return `
# Role
あなたは農業経費記録のサポートツールです。一般的な農業会計の知識をもとに、青色申告決算書（農業所得用）を参考にした勘定科目の候補を提案してください。税務上の最終判断はユーザー自身または税理士が行うものとします。
OCRテキストから、個人農家の青色申告決算書（農業所得用）を参考にした勘定科目の候補を提案してください。
...
- AIコメントは「提案科目の根拠」「判断が難しい場合はその理由」「農業経費とする際の注意事項」「ためになるワンポイント知識」などを200字程度を目安に、要点をシンプルにまとめて記述する。断定表現は避け「〜と考えます」「一般的には〜が使われます」「〜の可能性があります」など参考情報としてのニュアンスで記述すること。特記科目（最優先）の情報が提供されておりそれに基づき提案した場合は、その旨を明記する。
                        ...
                        items: {
                            ...
                            properties: {
                                ...
                                category: {
                                    ...
                                    format: 'enum',
                                    description: '提案する勘定科目名（一般的な農業会計の観点からの候補）',
                                    enum: [
                                        '作業用衣料費', '荷造運賃手数料', '小農具費',
