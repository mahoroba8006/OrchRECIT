...
前提として、読み込むレシートはすべて農業に関連のある支払いであるものとします。必ず01〜21のいずれかの科目に分類してください。「要確認」や「該当なし」は選択しないでください。
...
- 農業と家事の両用が多い品（洗剤・食料品・衣類全般）→ 農業利用の証跡が必要。必ず何らかの農業科目に割り当てること
- 明らかに家事費と思われるものも、強いて言えば合致する科目を選ぶこと
...
- **AIコメント（reason）は最大200文字程度**とし、「判断理由」「判断が難しい場合はその理由」「注意点」「ためになる情報や知識」「一般的に農業用と判断できない場合、農業経費とすることができる可能性がある条件」などに要点を絞り、ダラダラ書かず短く簡潔に記述すること。
`;
        ...
        config: {
            ...
            responseSchema: {
                ...
                properties: {
                    ...
                    reason: { type: Type.STRING, description: "AIコメント（判断理由、判断が難しい理由、注意点、役立つ知識、一般的に農業用と判断できない場合の経費算入の条件など。最大200文字程度で簡潔に）" }
                },
